// import 'package:booking_application/views/Games/GameViews/Football/football_summary.dart';
// import 'package:booking_application/views/Games/GameViews/Football/halftime.dart';
...
//     DateTime? secondHalfKickoffTime;
...
//     final now = DateTime.now();
//     print('Now: $now');
//           print('secondHalfKickoffTimeeeeeeeee: $secondHalfKickoffTime');

//     // Choose which kickoff time to use
//     DateTime? referenceKickoff;
//     if (currentStatus == 'second-half' && secondHalfKickoffTime != null) {
//       referenceKickoff = secondHalfKickoffTime!;
//           print('secondHalfKickoffTime: $secondHalfKickoffTime');

//     } else {
//       referenceKickoff = kickOffTime!;
//           print('referenceKickoff: $referenceKickoff');

//     }

//     // Convert to local if stored as UTC
//     final kickoff = referenceKickoff.isUtc
//         ? DateTime(
//             referenceKickoff.year,
//             referenceKickoff.month,
//             referenceKickoff.day,
//             referenceKickoff.hour,
//             referenceKickoff.minute,
//             referenceKickoff.second,
//             referenceKickoff.millisecond,
//             referenceKickoff.microsecond,
//           )
//         : referenceKickoff;

//     print('Kickoff used: $kickoff');

//     Duration difference = now.difference(kickoff);
//         if (currentStatus == 'second-half' && totalDuration != null) {
//       final halfTimeMinutes = (totalDuration / 2).round();
//       difference += Duration(minutes: halfTimeMinutes);
//       print('Added halftime ($halfTimeMinutes min). New difference: $difference');
//     }
...
//         if (data['secondHalfKickoffTime'] != null) {
//       try {
//         kickOffTime = DateTime.parse(data['secondHalfKickoffTime']);
//       } catch (e) {
//         print('Error parsing secondHalfKickoffTime: $e');
//       }
//     }
...
//       }
//     }
//         if (matchData!['secondHalfKickoffTime'] != null) {
//       try {
//         secondHalfKickoffTime = DateTime.parse(matchData!['secondHalfKickoffTime']);
//         _elapsedTime = _calculateElapsedTime();
//       } catch (e) {
//         print('Error parsing secondHalfKickoffTime: $e');
//       }
...
//                   // Filter out "out" players using scoreCard
//                   final activePlayers = players.where((player) {
//                     for (var teamScore in scoreCard) {
//                       if (teamScore['teamId'] == teamId) {
//                         final scorecardPlayers =
//                             teamScore['players'] as List? ?? [];
//                         for (var scorecardPlayer in scorecardPlayers) {
//                           if (scorecardPlayer['playerId'] ==
//                               player['playerId']) {
...
//                       }
//                     }
//                     return true; // if not found, assume active
//                   }).toList();
...
//                     ),
//                     subtitle: Text('${activePlayers.length} active players'),
//                     trailing: const Icon(Icons.arrow_forward_ios, size: 16),
//                     onTap: () {
//                       Navigator.pop(context);
//                       _showPlayerSelectionForCard(
//                           teamId, activePlayers, cardType);
//                     },
//                   );
//                 },
//               ),
...
//           context,
//           MaterialPageRoute(
//             builder: (context) => ViewMatchScreen(initialTabIndex: 3,),
//           ),
//         );
//       }
//     } catch (e) {
...
class _ScorecardScreenState extends State<ScorecardScreen> {
  ...
  Duration _calculateElapsedTime() {
    ...
    try {
      ...
      if (currentStatus == 'second-half' && totalDuration != null) {
        ...
        difference += Duration(minutes: halfTimeMinutes);
        print(
            'Added halftime ($halfTimeMinutes min). New difference: $difference');
      }
  ...
  void _updateFromSocketData(Map<String, dynamic> data) {
    ...
    if (data['secondHalfKickoffTime'] != null) {
      try {
        secondHalfKickoffTime = DateTime.parse(data['secondHalfKickoffTime']);
      } catch (e) {
        print('Error parsing secondHalfKickoffTime: $e');
      }
    }
    ...
    if (matchData!['secondHalfKickoffTime'] != null) {
      try {
        secondHalfKickoffTime =
            DateTime.parse(matchData!['secondHalfKickoffTime']);
        _elapsedTime = _calculateElapsedTime();
      } catch (e) {
        print('Error parsing secondHalfKickoffTime: $e');
      }
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      onTap: () {
                        Navigator.pop(context);
                        _showTeamSubstitutionModal(teamId, players);
                      },
                    );
                  },
                ),
  ...
  // NEW: Team Substitution Modal
  void _showTeamSubstitutionModal(String teamId, List players) {
    // Get playing players (isPlaying: true)
    final playingPlayers = players.where((player) {
      // Check if player is playing from the initial team configuration
      return player['isPlaying'] == true;
    }).toList();

    // Get bench players (isPlaying: false)
    final benchPlayers = players.where((player) {
      return player['isPlaying'] == false;
    }).toList();

    if (playingPlayers.isEmpty || benchPlayers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No substitution options available')),
      );
      return;
    }
                  ...
                  Container(
                    ...
                    child: DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'Select player to substitute out',
                      ),
                      items: playingPlayers
                          .map<DropdownMenuItem<String>>((player) {
                        final playerName = player['playerName'] ?? 'Unknown';
                        final playerId =
                            player['playerId'] as String; // Ensure it's String
                  ...
                  Container(
                    ...
                    child: DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(
                        border: InputBorder.none,
                        contentPadding: EdgeInsets.symmetric(horizontal: 12),
                        hintText: 'Select player to substitute in',
                      ),
                      items:
                          benchPlayers.map<DropdownMenuItem<String>>((player) {
                        final playerName = player['playerName'] ?? 'Unknown';
                    ...
                    child: ElevatedButton(
                      onPressed:
                          selectedPlayerOut != null && selectedPlayerIn != null
                              ? () {
                                  Navigator.pop(context);
                                  _makeSubstitution(teamId, selectedPlayerOut!,
                                      selectedPlayerIn!);
                                }
                              : null,
                      style: ElevatedButton.styleFrom(
  ...
  // NEW: Make Substitution API Call
  Future<void> _makeSubstitution(
      String teamId, String playerId, String newPlayerId) async {
    try {
      final payload = {
        "teamId": teamId,
        "playerId": playerId,
      ...
      print("🔄 Substitution payload: ${jsonEncode(payload)}");

      // Call your substitution API endpoint
      await MatchApiService.makeSubstitution(widget.matchId, payload);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Substitution made successfully!'),
            backgroundColor: Color(0xFF8B5CF6),
          ),
        );
      }
    } catch (e) {
      print('Error making substitution: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error making substitution: $e'),
              ...
              Flexible(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    // Filter only playing players for card selection
                    final playingPlayers = players.where((player) {
                      return player['isPlaying'] == true;
                    }).toList();

                    // Filter out "out" players using scoreCard
                    final activePlayers = playingPlayers.where((player) {
                      for (var teamScore in scoreCard) {
                        if (teamScore['teamId'] == teamId) {
                          final scorecardPlayers =
                              teamScore['players'] as List? ?? [];
  ...
  void _showPlayerSelectionModal(String teamId, List players, String action) {
    ...
    // Filter only playing players for goal updates
    final playingPlayers = players.where((player) {
      return player['isPlaying'] == true;
    }).toList();

    // Filter out players who are marked as "isOut" (sent off)
    List activePlayers = playingPlayers.where((player) {
      // Check in scoreCard for isOut status
      for (var teamScore in scoreCard) {
        if (teamScore['teamId'] == teamId) {
          final scorecardPlayers = teamScore['players'] as List? ?? [];
  ...
  // ... Rest of the existing methods remain the same (showHalfTimeModal, showFullTimeModal, etc.)
  // These include: _showHalfTimeModal, _showFullTimeModal, _updateMatchStatus,
  // _updateMatchCancel, _incrementScore, _decrementScore, _endGame,
  // _showEndGameDialog, _showStopDialog, _showStopOptionsDialog, _showCancelReasonDialog
