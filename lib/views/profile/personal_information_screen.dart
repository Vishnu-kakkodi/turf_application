...
//                             : provider.errorMessage ?? 'Failed to update profile image'),
...
//     // Use the new updateUserInfo method instead of updateProfile
...
class _PersonalInformationScreenState extends State<PersonalInformationScreen> {
  ...
  String? _localImagePath; // Store selected image path for preview
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      provider.loadUserProfile().then((_) {
        setState(() {
          _localImagePath = null; // Clear local preview after loading profile
        });
      });
    });
  }
                ...
                onTap: () async {
                  ...
                  try {
                    ...
                    if (image != null && mounted) {
                      ...
                      final success =
                          await provider.uploadProfileImage(image.path);
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(success
